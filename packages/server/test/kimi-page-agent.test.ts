...
import { createKimiPageAgentRoutes } from "../src/routes/kimi-page-agent.js";
import { MockClaudeSDK } from "../src/sdk/mock.js";

describe("Kimi Page Agent preflight", () => {
  it("builds embed URLs from forwarded sandbox origin and session target path", async () => {
    const routes = createKimiPageAgentRoutes();
    const targetPath =
    ...
    const res = await routes.request(
      `/preflight?targetPath=${encodeURIComponent(targetPath)}`,
      {
        headers: {
          ...
          "x-forwarded-host":
            "cdfc69-sandbox-session633f9874a3ca4b7e9f-3400.agent.alibaba-inc.com",
        },
      },
    );
    ...
    expect(json.capability).toBe("kimi-page-agent");
    expect(json.origin).toBe(
      "https://cdfc69-sandbox-session633f9874a3ca4b7e9f-3400.agent.alibaba-inc.com",
    );
    expect(json.target.kind).toBe("session");
    expect(json.urls.targetEmbedUrl).toBe(
      "https://cdfc69-sandbox-session633f9874a3ca4b7e9f-3400.agent.alibaba-inc.com/projects/L2hvbWUvYWRtaW4/sessions/fc624e64-ded8-4673-9105-f0fcb6600319?mode=kimi-page-agent&host=chrome-extension",
    );
    expect(json.urls.websocketUrl).toBe(
      "wss://cdfc69-sandbox-session633f9874a3ca4b7e9f-3400.agent.alibaba-inc.com/api/ws",
    );
  });
  ...
  it("preserves existing target query params when adding KPA params", async () => {
    ...
    expect(embed.searchParams.get("mode")).toBe("kimi-page-agent");
  ...
  it("is reachable through createApp even when auth is enabled", async () => {
    ...
    const res = await app.request("/api/kimi-page-agent/preflight");
    const json = await res.json();
