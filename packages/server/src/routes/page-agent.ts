...
const PAGE_AGENT_MODE = "page-agent";
const PAGE_AGENT_HOST = "chrome-extension";
...
export interface PageAgentRoutesOptions {
  authService?: AuthService;
  authDisabled?: boolean;
  desktopAuthToken?: string;
}

export function createPageAgentRoutes(
  options: PageAgentRoutesOptions = {},
): Hono {
  const app = new Hono();
  ...
  app.get("/preflight", (c) => {
    ...
    const targetEmbedUrl = withPageAgentParams(target);
    const newSessionEmbedUrl = withPageAgentParams(
    ...
    const checks: PreflightCheck[] = [
      {
        id: "yep_server",
        status: "pass",
        message: "Reached the Yep Anywhere Page Agent preflight endpoint.",
      },
      {
        id: "host_allowed",
        status: "pass",
...
function withPageAgentParams(url: URL): string {
  const next = new URL(url);
  next.searchParams.set("mode", PAGE_AGENT_MODE);
  next.searchParams.set("host", PAGE_AGENT_HOST);
...
function getAuthAdvice(options: PageAgentRoutesOptions) {
